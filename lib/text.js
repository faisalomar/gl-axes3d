...
proto.update = function(bounds, labels, labelFont, ticks, tickFont) {
  ...
  for(var d=0; d<3; ++d) {
    ...
    console.log("0.5*(bounds[0][d]+bounds[1][d])=", 0.5*(bounds[0][d]+bounds[1][d]));
...
//    console.log("VERTEX_SIZE=", VERTEX_SIZE);
//    console.log("tickOffset[d]=", tickOffset[d]);

    tickCount[d] = ((data.length/VERTEX_SIZE)|0) - tickOffset[d]

//    console.log("tickCount[d]=", tickCount[d]);
  }
...
//  console.log("this.tickCount=", this.tickCount[0], this.tickCount[1], this.tickCount[2]);
}
